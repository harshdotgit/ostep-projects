...
int request_parse_uri(char *uri, char *filename, char *cgiargs) {
  ...
  if (!strstr(uri, "cgi")) {
    ...
    // 2. looks for '?' and stores everyhting after ? imm cigargs. If nothing
    // after ?, cigargs is empty the entire uri is formated as a string and
    // stored in a buffer (filename) dynamic
    ptr = index(uri, '?');
    if (ptr) {
